// Lines98_Mouse_Full.cpp — complete Lines 98 game for Engine++.
// Replace the contents of the script that currently contains onStart/onUpdate
// with this entire file. Mouse, touch-style clicks, arrows and WASD are supported.
...
int mouseCell(Scene& scene) {
    Entity* map = grid(scene);
    if (!map || map->cellSize <= 0) return -1;
    int x = qFloor((scene.input.mouseX - map->x) / map->cellSize);
    int y = qFloor((scene.input.mouseY - map->y) / map->cellSize);
    return inside(x, y) ? index(x, y) : -1;
}
...
void lines98Update(Scene& scene, float /*dt*/) {
    ...
    // mouseDown plus our own edge detector is reliable even at a low frame rate.
    const bool mousePressed = scene.input.mouseDown;

    // Hover follows the mouse only while it is over the Grid Map.
    int pointedCell = mouseCell(scene);
    if (pointedCell >= 0) game.cursor = pointedCell;
    ...
    if (mousePressed && !game.oldMousePressed && pointedCell >= 0)
        activateCell(scene);
    ...
    game.oldDown = down;
    game.oldSpace = space;
    game.oldMousePressed = mousePressed;
    draw(scene);
}

// These are the only Engine++ entry points. Keep exactly one pair in the project.
